...
export default function CustomerPortal() {
  ...
  return <div className="portal-shell"><div className="container max-w-6xl py-8 md:py-12"><div className="flex flex-wrap items-center justify-between gap-4"><Link href="/" className="back-link"><ArrowLeft className="size-4" /> Back to VOLAMP</Link><div className="flex items-center gap-3 text-sm font-bold"><span className="portal-avatar">{(user.name || user.email || "V").charAt(0).toUpperCase()}</span>{user.name || user.email}</div></div><div className="portal-hero"><div><p className="eyebrow text-sun">Your private customer space</p><h1>Welcome back,<br /><span>{user.name?.split(" ")[0] || "bright mind"}.</span></h1><p>Keep your project moving without the paperwork shuffle.</p></div><div className="portal-hero-icon"><PackageSearch className="size-12" /></div></div>
